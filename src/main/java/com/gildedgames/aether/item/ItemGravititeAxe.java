...
public class ItemGravititeAxe extends TemplateToolBase {
    ...
    public ItemGravititeAxe(final @NotNull Identifier identifier, final ToolMaterial enumtoolmaterial) {
        super(identifier, 3, enumtoolmaterial, ItemGravititeAxe.blocksEffectiveAgainst);
    }
        ...
        if (movingobjectposition.type == HitType.TILE) {
            ...
            if (!level.isServerSide) {
                ...
                for (int n = 0; n < ItemGravititeAxe.blocksEffectiveAgainst.length; ++n) {
                    if (blockID == ItemGravititeAxe.blocksEffectiveAgainst[n].id) {
    ...
    static {
        ItemGravititeAxe.blocksEffectiveAgainst = new BlockBase[] { AetherBlocks.SKYROOT_PLANKS, AetherBlocks.LOG };
